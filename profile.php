<?php
	...
	function add(){
		...
		if ($conf) {
			...
			echo "<h3>Error al enviar solicitud ...</h3>";
		}
	}
	...
	function cancel(){
		...
		if ($conf) {
			...
			echo "<h3>Error al remover solicitud...</h3>";
		}
	}
	...
	function remove(){
		...
		if ($conf) {
			...
			echo "<h3>Error al eliminar la amistad ...</h3>";
		}
	}
...
<body>
	<?php
		if ($saber["img"]=="") {
			echo '<img src="img/Anonimo.png" id="profile">';
		}else{
	...
	<div id="menu">
		<form method="POST">
		<h2><?php echo $saber['nome']." (".$saber['apelido'].")<br>".$saber['edad']." años<br> Estado Civil: ".$saber['estado']."<br>Biografía: ".$saber['desc']."<br>Enfermedades: ".$saber['enf']; ?></h2><br />
		<?php
			...
			if (mysql_num_rows($amigos)>=1 AND $amigoss["aceite"]=="sim") {
				echo '<input type="submit" value="Remover amigo" name="remover"><input type="submit" name="chat" value="Conversar"><input type="submit" name="denunciar" value="Reportar">';
			}elseif (mysql_num_rows($amigos)>=1 AND $amigoss["aceite"]=="nao" AND $amigoss["para"]==$login_cookie){
				echo '<input type="submit" value="Aceitar pedido" name="aceitar"><input type="submit" name="chat" value="Conversar"><input type="submit" name="denunciar" value="Reportar">';
			}elseif (mysql_num_rows($amigos)>=1 AND $amigoss["aceite"]=="nao" AND $amigoss["de"]==$login_cookie){
				echo '<input type="submit" value="Cancelar Solicitud" name="cancelar"><input type="submit" name="chat" value="Conversar"><input type="submit" name="denunciar" value="Reportar">';
			}else{
				echo '<input type="submit" value="Adicionar amigo" name="add"><input type="submit" name="chat" value="Conversar"><input type="submit" name="denunciar" value="Reportar">';
			}
		?>
		</form>
	</div>
	<?php
		...
		if ($amigos==1) {
			...
			echo '<div class="pub" id="'.$id.'">
						<p>Aviso sobre las amistades ...</p>
						<span>Tienes que ser amigo(a) de '.$nome.' para poder ver sus publicaciones ...</span><br />
					</div>';
		}
	?>
	<br />
	<div id="footer"><p>&copy; Grandpa Services, 2018</p></div><br />
</body>
</html>