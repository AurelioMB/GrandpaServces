<?php
	...
	if (isset($_GET['id'])) {
		...
		if ($conf) {
			...
			echo "<h3>Error al aceptar amistad ...</h3>";
		}
	}
...
<body>
	...
	<?php
		if (mysql_num_rows($pubs)>=1) {
			echo "<h3>Tus solicitudes de amistad:</h3>";
			while ($pub=mysql_fetch_assoc($pubs)) {
				...
				echo '<div class="pub" id="'.$id.'">
					<span>'.$nome.' quiere ser tu amigo amigo/a</span><br />
					<p><a href="profile.php?id='.$saber['id'].'">Ver perfil de '.$nome.'</a></p><br />
					<a href="notificacoes.php?id='.$saber['id'].'"><input type="submit" value="Aceptar" name="add"></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="notificacoes.php?remove='.$saber['id'].'"><input type="submit" value="Rechazar" name="remove"></a>
					<br /><br />
				</div>';
			}
			echo "<br /><h3>No tienes más solicitudes de amistad ...</h3><br /><br />";
		}
	?>
	<br />
	<?php
		if (mysql_num_rows($notificacoes)>=1) {
			echo "<h3>Tus notificaciones:</h3>";
			while ($not=mysql_fetch_assoc($notificacoes)) {
				...
				if ($not['tipo']=="1") {
					echo '<br /><div class="not" id="'.$id.'">
						<a href="myprofile.php?id='.$not['post'].'">'.$nome.' Le gustó tu publicación</a>
					</div>';
				}elseif($not['tipo']=="2"){
					echo '<br /><div class="not" id="'.$id.'">
						<a href="comentarios.php?id='.$not['post'].'">'.$nome.' Comento en tu publicación</a>
					</div>';
				}
			}
			echo "<br /><h3>No tienes más notificaciones ...</h3>";
		}else{
			echo "<br /><h3>No tienes notificaciones ...</h3>";
		}
	?>
	<br />
