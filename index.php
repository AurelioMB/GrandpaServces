<?php
	...
	if (isset($_POST['publish'])) {
		if ($_FILES["file"]["error"] > 0) {
			...
			if ($texto == "") {
				echo "<h3>¡Tienes que escribir algo antes de publicar!</h3>";
			}else{
				...
				if ($data) {
					...
					echo "Algo salio mal... Intente más tarde";
				}
			}
		}else{
			$n = rand(0, 1000000);
...
<body>
	...
	<div id="publish">
		<form method="POST" enctype="multipart/form-data">
			<br />
			<textarea placeholder="Escribe una nueva publicación...
O solicita uno de los siguientes servicios:
- Acompañamiento
- Servicios de Salud
- Nutrición
- Labores Domesticos" name="texto"></textarea>
			<label for="file-input">
				<img src="img/imagegrey.png" title="Insertar una fotografía" />
			</label>
	...
	<?php
		while ($pub=mysql_fetch_assoc($pubs)) {
			...
			if ($pub['imagem']=="") {
				...
				if ($do_email_check >= 1) {
					$loves = $loves - 1;
					echo '<p><a href="index.php?unlove='.$id.'">Me gusta</a> | Tu y mas '.$loves.' le gusto esto</p>';
				}else{
					echo '<p><a href="index.php?love='.$id.'">Me gusta</a> | '.$loves.' le gusto esto</p>';
				}
				echo '</div>';
			}else{
				echo '<div class="pub" id="'.$id.'">
